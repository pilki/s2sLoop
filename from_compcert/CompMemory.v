...
Transparent encode_val.
  ...
  destruct chunk'; auto. 
  destruct chunk'; auto. dintuition.
  omega.
Qed.
...
Proof.
  unfold free in FREE. destruct (range_perm_dec m1 bf lo hi Freeable); congruence.
Qed.
...
Proof.
  ...
  destruct (zlt lo hi). dintuition. right. omega. 
Qed.
...
Proof.
  ...
  assert (perm m1 b ofs p <-> perm m2 b ofs p) by (rewrite H; dintuition).
  assert (perm m1 b ofs p0 <-> perm m2 b ofs p0) by (rewrite H; dintuition).
...
Proof.
  ...
  unfold perm, perm_order'.
    rewrite <- H; destruct (mem_access m1 b ofs); try destruct p; dintuition.
  contradict n.
  intros ofs' ?; assert (ofs'=ofs) by omega; subst ofs'.
  unfold perm. destruct (mem_access m1 b ofs); try destruct p; dintuition.
Qed.
